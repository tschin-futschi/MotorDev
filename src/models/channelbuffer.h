...
// - 显示数据环（uiRing）：对原始数据做中值降采样后的结果，供绘图控件使用
//
// 降采样策略：每 bucketWidth 个原始样本取中值，写入 uiRing。
// 这样可以在高采样率下保持 UI 绘制性能，同时保留信号特征（中值比均值更抗尖刺）。
//
...
/// @brief 单通道数据缓冲区
///
/// 双层环形缓冲区：raw 层接收原始采样，经中值降采样后写入 UI 层供绘图使用。
/// 非线程安全，仅在主线程中使用。
class ChannelBuffer {
public:
