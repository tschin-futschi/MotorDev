...
### 统一配置文件存取（各页面功能参数）
- `src/services/appconfigservice` `[配置]` — **统一配置文件存取服务**：把各页面功能参数采集成一个 JSON 文件（手动 Write）/ 从 JSON 回填各页面（手动 Read）。写入仍**全手动**（参数变化不自动保存），路径由用户在配置页 ConfigFile 指定。**路径记忆 + 启动自动回填**：持久化逻辑在 MainWindow（监听 `finished(ok,path)`，Write/Read 成功即把路径写 QSettings `config/lastFile`；启动时填回输入框并触发一次 `onReadRequested` 自动回填，文件缺失/损坏记日志并优雅跳过，本服务自身不读写 QSettings）。结构 `{version:1, device, registers, scope, flash, flashStore}`。Read 语义：**只回填参数值，不触发任何串口动作**（不连接/下发/写寄存器/启动采样/烧录），启动自动回填同样零串口副作用。MainWindow 持有，接收 ConfigTab 的 readConfigRequested/writeConfigRequested，编排各 tab 的 `collectXxx()/applyXxx()`；缺段跳过 + 日志，version 不符仅告警尽力恢复；logMessage 经 MainWindow 路由到 LogPanel
- `src/tabs/configtab` `[UI-Tab]` — Config File 行（`m_filePathEdit` QLineEdit 路径输入 + Browse/Write/Read）：Browse 弹 `*.json` 文件框回填路径；Read/Write 发 `readConfigRequested/writeConfigRequested(path)` 信号交 AppConfigService；`setConfigFilePath(path)` 供 MainWindow 启动时把上次路径填回输入框（仅设文本，不读写文件/不发信号）；自身 `collectDeviceConfig()/applyDeviceConfig()` 负责 device 段（串口/波特率/IC/从机/PMIC 三路电压，apply 只回填不下发）
...
- `src/services/dw9786oisresetservice` `[通信]` — DW9786 上电 OISReset 服务：DW9786 上电默认 Sleep（寄存器全读 0xFFFF / 不自举），MainWindow 在「IC 连接成功（用户 I2C 扫描 → 选 SlaveID → 点 IC「连接」、从机地址设置成功 → `ConfigTab::motorIcConnected`）&& 当前 Select IC=DW9786」时调 `requestOisReset()`（2026-06-01 起由原「串口连接成功」改为此触发点）；经 `invokeMethod(QueuedConnection)` 投递到 SerialManager 工作线程，复用 `Dw9786Bridge` + vendor `RamWriteA`（0x30 透传 slave 0x24）执行用户口述权威 9 步序列（0xE000=0 → 0xE000=1 → 0xE2FC=0xAC1E,0xE164=8,0xE2FC=0 → 0xE004=1，含 10/10/5/1/100ms 延时，延时用 `QThread::msleep`）；**不调用 vendor 自带的更短 `dw9786_reset()`**；`logMessage` 信号回主线程进 LogPanel
