...
- 标签 "目标 IC：" 11px 常规字体
- IC 下拉框宽度 `Style::Size::FwFlashIcComboW = 200`，**只读（disabled）**：目标 IC 单向跟随配置页 Select IC（`DeviceContext`），不在本页单独选择；无"请选择目标 IC..."占位项，始终显示配置页当前 IC（默认 `AW86008`），tooltip 提示"请在配置页修改"
- 后续显示选中策略的 `icDescription()`（`Style::Color::MutedText`，11px），吃掉行内剩余空间
- 下拉框列表按 `FlashStrategyRegistry` 注册顺序：`AW86008` / `AW86100` / `DW9786` / `DW9788`（与配置页 Select IC 一致）
...
每款 IC 一个 `FlashStrategy` 子类，`flash()` 函数体留空（当前为 1 KB / 100 ms 的联调用模拟实现），由用户后续填入真实烧录算法。AW86100 与 AW86008 烧录算法等同，因此 `AW86100Strategy` 直接继承 `AW86008Strategy`，仅 override `icModel()` / `icDescription()`。
