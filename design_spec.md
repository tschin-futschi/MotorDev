...
```
整体缩放: 0.62（建议用 QGraphicsProxyWidget 实现）
表格类型: QTableWidget 或自定义 QAbstractTableModel

列结构（每组，重复 4 次）:
  描述列: 15%，左对齐，可编辑 QLineEdit
  地址列: 6%，居中，可编辑 QLineEdit，monospace
  值列:   7%，居中，可编辑 QLineEdit，monospace，蓝色
  读列:   3%，居中，QPushButton，蓝色系
  写列:   3%，居中，QPushButton，橙色系

行高: 19px
表头高度: 24px
组分隔线: 第 5、10、15 列左边框 2px solid #aaa
单元格 padding: 0
```
